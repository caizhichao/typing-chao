...
std::string g_app_name = "rime.typing-dongnanya";
...
void initializeRime(const std::string& sharedDataDirectory,
  ...
  const auto tablePath = userDataPath / "luna_pinyin.table.bin";
  ...
  if (needsDeploy) {
    ...
    if (!g_rime->deploy_config_file("default.yaml", "config_version") ||
        !g_rime->prebuild()) {
      NSLog(@"TypingDongnanya failed to prepare Rime data.");
...
- (instancetype)initWithSharedDataDirectory:(NSString *)sharedDataDirectory
  ...
  g_rime->select_schema(_sessionId, "luna_pinyin");
  return self;
}
