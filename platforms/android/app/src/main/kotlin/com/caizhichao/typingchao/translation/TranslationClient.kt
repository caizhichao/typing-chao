package com.caizhichao.typingchao.translation
...
// 翻译客户端只访问 DeepSeek 官方 HTTPS 接口，Key 从本地设置缓存读取。
class TranslationClient(private val settingsStore: SettingsStore) {
    fun newCall(sourceText: String, targetLanguage: TargetLanguage): TranslationCall {
        return TranslationCall(sourceText, targetLanguage, settingsStore.deepSeekAPIKey)
    }
    ...
    class TranslationCall(
        ...
        fun execute(): Result<String> {
            ...
            val endpointUrl = runCatching { URL(deepSeekEndpoint) }
                .getOrElse { return Result.failure(TranslationException.InvalidConfiguration) }
            val configuredAPIKey = deepSeekAPIKey
                ?: return Result.failure(TranslationException.MissingAPIKey)
            ...
            return try {
                ...
                connection.setRequestProperty("Authorization", "Bearer $configuredAPIKey")
    ...
    sealed class TranslationException : Exception() {
        ...
        data object MissingAPIKey : TranslationException()
