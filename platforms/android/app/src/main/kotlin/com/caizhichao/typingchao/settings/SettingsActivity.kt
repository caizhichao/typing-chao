...
import com.caizhichao.typingchao.R
...
class SettingsActivity : Activity() {
    ...
    override fun onPause() {
        ...
        settingsStore.setDeepSeekAPIKey(deepSeekAPIKeyInput.text.toString())
        super.onPause()
    }
    ...
    private fun buildContentView(): View {
        ...
        scrollContent.addView(sectionText("DeepSeek API Key"))
        deepSeekAPIKeyInput = EditText(this).apply {
            hint = "手动输入 DeepSeek API Key"
            inputType = InputType.TYPE_CLASS_TEXT or InputType.TYPE_TEXT_VARIATION_PASSWORD
            setSingleLine(true)
            setText(settingsStore.deepSeekAPIKey.orEmpty())
            textSize = 15f
        }
        scrollContent.addView(deepSeekAPIKeyInput, LinearLayout.LayoutParams(
            LinearLayout.LayoutParams.MATCH_PARENT,
            dp(54),
        ))
    ...
    private fun refreshInstallationStatus() {
        ...
        installationStatusView.text = when {
            selectedValue -> "当前状态：Typing Chao 正在使用"
