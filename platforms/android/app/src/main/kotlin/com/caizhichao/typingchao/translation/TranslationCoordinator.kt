...
class TranslationCoordinator(
    ...
    context: Context,
    private val translationClient: TranslationClient = TranslationClient(SettingsStore(context)),
) {
    interface Listener {
    ...
    private fun userMessage(errorValue: Throwable): String {
        return when (errorValue) {
            TranslationClient.TranslationException.MissingAPIKey -> "请先在输入法设置中配置 DeepSeek Key"
            TranslationClient.TranslationException.InvalidConfiguration -> "DeepSeek 翻译地址无效，请更新应用后再试"
