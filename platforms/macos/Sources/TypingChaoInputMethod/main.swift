...
if CommandLine.arguments.contains("--register-input-source") {
    ...
    if registrationStatus == noErr {
        print("已注册 Typing Chao 输入法包，尚未切换当前输入法。")
    } else {
        print("注册 Typing Chao 输入法失败，状态码：\(registrationStatus)")
    }
...
NSApplication.shared.delegate = TypingChaoApplicationDelegate.shared
